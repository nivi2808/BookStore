...
# Create OAuth2PasswordBearer instance to extract the token from the header
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token")
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    try:
        ...
        email = payload.get("sub")  # 'sub' is typically the user identifier (e.g., email)
...
@router.post("/api/auth/signup", response_model=schemas.UserResponse, status_code=200)
...
@router.post("/api/auth/signin",  status_code=200)
